...
&nbsp;&nbsp;&nbsp;&nbsp;In the face of this problem, one approach is to use a proof of computational integrity test generated by [risc0](https://www.risczero.com/) ZK 
virtual machine. This allows the addressing of more complex problems with solutions whose verification 
...
upload rust risc0 compatble code. This code is then distributed with different arguments among the various clients connecting to the server 
to donate their computing power.


&nbsp;&nbsp;&nbsp;&nbsp;The donor will download the code of the project they want to contribute to, execute it in the Risc0 ZKVM, 
which will generate the proof of integrity, which will then be sent to the system server. The server will store the uploaded proof 
until the user that uploaded the program for execution requests and properly verifies it, accepting the output in case of a successful verification result.
...
the client's code. This may take several minutes. This does not leave eny residual containers.
- `make clear_build`: deletes the container's volume, the execution of `run_all` after this command will trigger a client recompiling.
- `make clear_docker`: clears the containers and deletes the images created for the `uploader client`. Note that cached images are not 
deleted with this, and should be deleted with the command `docker image prune -a`, this will delete ANY image without an associated container,
even if it is not related to this project. This should save at least 7 GB of disk space, however, take into account that the `executor client`
makes used of the cached image generated by the build command.
- `make clear_all`: runs `clear_docker` and `clear_build`.
