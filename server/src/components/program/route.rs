...
pub fn program_router(path_prefix: &str) -> Scope {
    ...
    web::scope(path_prefix).
        ...
        // route("download/{organization_id}/{program_id}", web::get().to(ProgramController::download_program)).
        route("download/{program_id}", web::get().to(ProgramController::download_program)).
